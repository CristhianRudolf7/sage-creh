...
def main():
    ...
    # Compare serialized JSON values: in-memory SQLite rows are tuples, while
    # their historical JSON representation necessarily uses lists.
    task_data = json.loads(json.dumps([dict(id=task.id, family=task.family, public=task.public, expected=task.expected, sql_fixtures=task.sql_fixtures) for task in tasks]))
